...
// Cleaner, bulletproof rider admin controller with single availability flag (isAvailable)
  ...
  try {
    ...
    if (search?.trim()) {
      ...
      query.$or = [
        { name: regex },
        { phone: regex },
        { 'riderDocuments.cnicNumber': regex },
        { 'riderDocuments.vehicleNumber': regex },
      ];
    }

    const [riders, total] = await Promise.all([
      User.find(query)
        .select('name phone email riderStatus riderDocuments isAvailable rating totalDeliveries earnings createdAt')
        .sort({ createdAt: -1 })
        .skip((page - 1) * limit)
        .limit(+limit)
        .lean(),
...
const updateRiderStatus = async (req, res) => {
  ...
  try {
    const rider = await User.findOneAndUpdate(
      ...
      { riderStatus, isAvailable: riderStatus === 'approved' },
      { new: true }
    ).select('name phone fcmToken riderStatus isAvailable');
    ...
    // Real-time update
    if (io) io.to(`rider:${rider._id}`).emit('statusChanged', { riderStatus, isAvailable: rider.isAvailable });
    ...
    res.json({
      ...
      rider: { id: rider._id, name: rider.name, riderStatus, isAvailable: rider.isAvailable },
    });
  } catch (err) {
...
const getRiderStats = async (req, res) => {
  try {
    ...
    const result = stats[0] || { total: 0, available: 0, approved: 0, pending: 0, rejected: 0 };
...
const approveRider = async (req, res) => {
  ...
  try {
    ...
    if (user.role === 'rider') return res.status(400).json({ success: false, message: 'User is already a rider' });
    if (user.riderStatus !== 'pending') return res.status(400).json({ success: false, message: 'No pending application found' });
    ...
    if (io) {
      io.to('admin').emit('riderApproved', { riderId: user._id, name: user.name, phone: user.phone });
      io.to(`rider:${user._id}`).emit('applicationApproved');
    }

    if (user.fcmToken) {
      await sendNotification(user.fcmToken, "Congratulations! 🎉", "Your rider application is approved! Go online and start earning.");
    }
...
const rejectRider = async (req, res) => {
  ...
  if (!reason || reason.trim().length < 10) return res.status(400).json({ success: false, message: 'Reason must be at least 10 characters' });

  if (!isValidObjectId(req.params.id)) return res.status(400).json({ success: false, message: 'Invalid user ID' });

  try {
    const user = await User.findById(req.params.id);
    if (!user || user.riderStatus !== 'pending') return res.status(400).json({ success: false, message: 'Pending application not found' });
    ...
    if (user.fcmToken) await sendNotification(user.fcmToken, "Application Rejected", reason.trim());
...
// ============================================================
// 7. Promote Any User to Rider (Admin Only)
// ============================================================
const promoteUserToRider = async (req, res) => {
  ...
  try {
    ...
    if (['admin', 'kitchen'].includes(user.role)) return res.status(403).json({ success: false, message: 'Cannot promote staff/admin' });
    ...
    Object.assign(user.riderDocuments, { vehicleType: 'bike', vehicleNumber: '' }, req.body.riderDocuments || {});
    ...
    if (user.fcmToken) await sendNotification(user.fcmToken, "Welcome to the Team!", "You are now a rider!");

    res.json({ success: true, message: 'User promoted to rider', rider: { id: user._id, name: user.name, phone: user.phone } });
  } catch (err) {
...
const blockRider = async (req, res) => {
  const { reason } = req.body;
  if (!reason || reason.trim().length < 5) return res.status(400).json({ success: false, message: 'Reason required (min 5 chars)' });
  if (!isValidObjectId(req.params.id)) return res.status(400).json({ success: false, message: 'Invalid rider ID' });

  try {
    const rider = await User.findOneAndUpdate(
      { _id: req.params.id, role: 'rider' },
      { isBlocked: true, isActive: false, isAvailable: false, blockReason: reason.trim(), blockedAt: new Date() },
      { new: true }
    ).select('name phone');

    if (!rider) return res.status(404).json({ success: false, message: 'Rider not found' });

    if (io) io.to(`rider:${rider._id}`).emit('forceLogout', { message: 'Your account has been blocked', reason: reason.trim() });
...
const unblockRider = async (req, res) => {
  ...
  try {
    const rider = await User.findOneAndUpdate(
      ...
      { $set: { isBlocked: false, isActive: true, isAvailable: true }, $unset: { blockReason: "", blockedAt: "" } },
      { new: true }
    ).select('name phone fcmToken');

    if (!rider) return res.status(404).json({ success: false, message: 'Blocked rider not found' });

    if (io) io.to(`rider:${rider._id}`).emit('accountRestored');
    if (rider.fcmToken) await sendNotification(rider.fcmToken, "Account Restored", "Welcome back! You can now accept orders.");
...
const softDeleteRider = async (req, res) => {
  ...
  try {
    const rider = await User.findOneAndUpdate(
      ...
      { isDeleted: true, isActive: false, isAvailable: false, deletedAt: new Date() },
      { new: true }
    );
...
const restoreRider = async (req, res) => {
  ...
  try {
    const result = await User.updateOne(
      ...
      { $set: { isDeleted: false, isActive: true, isAvailable: true }, $unset: { deletedAt: "" } }
    );

    if (result.modifiedCount === 0) return res.status(404).json({ success: false, message: 'Deleted rider not found' });

    const rider = await User.findById(req.params.id).select('name phone fcmToken');
    if (io) io.to(`rider:${rider._id}`).emit('accountRestored');
    if (rider?.fcmToken) await sendNotification(rider.fcmToken, "Account Restored", "Your account has been reactivated.");
...
const permanentlyBanRider = async (req, res) => {
  const { reason } = req.body;
  if (!reason || reason.trim().length < 10) return res.status(400).json({ success: false, message: 'Ban reason required (min 10 chars)' });
  if (!isValidObjectId(req.params.id)) return res.status(400).json({ success: false, message: 'Invalid rider ID' });

  try {
    const rider = await User.findOneAndUpdate(
      { _id: req.params.id, role: 'rider', isPermanentlyBanned: { $ne: true } },
      { isPermanentlyBanned: true, isBlocked: true, isActive: false, isAvailable: false, banReason: reason.trim(), bannedAt: new Date() },
      { new: true }
    );
...
const assignOrderToRider = async (req, res) => {
  ...
  if (!isValidObjectId(orderId) || !isValidObjectId(riderId)) return res.status(400).json({ success: false, message: 'Invalid ID format' });
  ...
  try {
    ...
    if (!order) return res.status(404).json({ success: false, message: 'Eligible order not found' });

    const rider = await User.findById(riderId).select('name phone fcmToken riderDocuments isAvailable');
    if (!rider || rider.role !== 'rider') return res.status(404).json({ success: false, message: 'Rider not found' });

    const activeOrder = await Order.findOne({ rider: riderId, status: { $in: ['confirmed', 'preparing', 'out_for_delivery'] } });
    if (activeOrder) return res.status(400).json({ success: false, message: `Rider is busy with order #${activeOrder._id.toString().slice(-6).toUpperCase()}` });

    const wasUnavailable = !rider.isAvailable;
    if (wasUnavailable) await User.updateOne({ _id: riderId }, { isAvailable: true });
    ...
    if (io) {
      ...
      io.to('admin').emit('orderAssigned', { orderId: order._id, shortId, riderName: rider.name });
    }

    if (rider.fcmToken) await sendNotification(rider.fcmToken, 'New Order Assigned!', `Order #${shortId} • PKR ${order.finalAmount} • Pick up now!`);
    if (order.customer?.fcmToken) await sendNotification(order.customer.fcmToken, 'Rider Assigned', `${rider.name} is coming to pick up your order`);

    res.json({
      success: true,
      message: 'Order assigned successfully',
      forcedAvailable: wasUnavailable,
...
// ============================================================
// 16. Get Available Riders
// ============================================================
const getAvailableRiders = async (req, res) => {
  try {
    const { area, page = 1, limit = 50 } = req.query;
    const activeStatuses = ['confirmed', 'preparing', 'out_for_delivery'];

    const busyRiderIds = await Order.distinct('rider', { rider: { $ne: null }, status: { $in: activeStatuses } });
    ...
    if (area) {
      // query['currentArea'] = area; // future
    }

    const [riders, total] = await Promise.all([
      User.find(query)
        .select('name phone rating totalDeliveries earnings isAvailable currentLocation riderDocuments.vehicleType createdAt')
        .sort({ rating: -1, totalDeliveries: -1, createdAt: -1 })
        .skip((page - 1) * limit)
        .limit(+limit)
        .lean(),
      User.countDocuments(query),
    ...
    if (io && total > 0) io.to('admin').emit('availableRidersCount', { count: total });
