// src/models/wallet/WithdrawalRequest.js
const mongoose = require('mongoose');

const withdrawalRequestSchema = new mongoose.Schema({
  user: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
    required: true,
    index: true
  },
  
  wallet: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'Wallet',
    required: true
  },
  
  amount: {
    type: mongoose.Schema.Types.Decimal128,
    required: true,
    min: 0
  },
  
  status: {
    type: String,
    enum: ['pending', 'processing', 'approved', 'rejected', 'completed', 'cancelled'],
    default: 'pending',
    index: true
  },
  
  requestedAt: {
    type: Date,
    default: Date.now
  },
  
  requestedBy: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User' // usually the rider themselves
  },
  
  processedAt: Date,
  processedBy: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User' // admin/finance
  },
  
  // Bank / Payment details (should be encrypted in production)
  bankDetails: {
    bankName: String,
    accountTitle: String,
    accountNumber: String,      // encrypt in real prod
    iban: String,
    branchCode: String
  },
  
  // For Easypaisa/JazzCash etc.
  mobileWalletNumber: String,
  paymentMethod: {
    type: String,
    enum: ['bank_transfer', 'easypaisa', 'jazzcash', 'cash_pickup'],
    required: true
  },
  
  referenceNumber: String,      // transaction ref after completion
  rejectionReason: String,
  adminNote: String,
  
  metadata: mongoose.Schema.Types.Mixed
}, {
  timestamps: true
});

// Indexes for fast queries
withdrawalRequestSchema.index({ status: 1, requestedAt: -1 });
withdrawalRequestSchema.index({ user: 1, status: 1 });
withdrawalRequestSchema.index({ processedAt: -1 });

module.exports = mongoose.models.WithdrawalRequest || 
  mongoose.model('WithdrawalRequest', withdrawalRequestSchema);